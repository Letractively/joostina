...
if(!defined('_INSTALL_CHECK')) {
	// этот метод отличаетс€ от подобного в 1.1, т.к. отличаетс€ обработка сессий
	$_s = session_id();
	if( !isset($_s)) {
	...
	if(class_exists('mosUser') && $database != null ) {
		...
		// проверка наличи€ строки сессии в базе данных
...
if(!defined('_ADMIN_OFFLINE') || defined('_INSTALL_CHECK')) {
	...
	if($database != null) {
		// получение названи€ шаблона сайта по умолчанию
