...
function system_info($version, $option) {
	...
	$version = $_VERSION->CMS.' <strong style="color: red;">'.$_VERSION->RELEASE.
		'.'.$_VERSION->DEV_LEVEL.'</strong> '.$_VERSION->DEV_STATUS.' [ '.$_VERSION->CODENAME.
		' ] '.$_VERSION->RELDATE.' '.$_VERSION->RELTIME.' '.$_VERSION->RELTZ;
	$width = 400;
	$tabs = new mosTabs(0);
?>
	<br />
