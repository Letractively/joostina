...
if($mosConfig_sef) {
    ...
    $new_REQUEST_URI = str_replace(JPATH_SITE, '', 'http://www.'.$_SERVER_HTTP_HOST.$_SERVER['REQUEST_URI']);
    $new_REQUEST_URI = str_replace('.html', '', $new_REQUEST_URI);
    //$new_REQUEST_URI = $_SERVER['REQUEST_URI'];
    $url_array = explode('/', $new_REQUEST_URI );
