...
function pollAddVote($uid) {
	...
	if(!$sessioncookie) {
		...
		echo '<input class="button" type="button" value="'._CMN_CONTINUE.
	...
	if($redirect) {
		...
		echo '<input class="button" type="button" value="'._BUTTON_RESULTS.
			'" onClick="window.location=\''.sefRelToAbs('index.php?option=com_poll&task=results&id='.
			$uid).'\'">';
		echo '</form>';
	}
}
