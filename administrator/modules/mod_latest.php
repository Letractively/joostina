...
$rows = $database->setQuery($query, 0, $limit)->loadObjectList();
?>
<table class="adminlist">
    ...
    foreach ($rows as $row) {
        ...
        $author = $row->created_by_alias ? $row->created_by_alias : $row->name;

        $title = $row->state ? _PUBLISHED : _UNPUBLISHED;
    ...
    <tr class="row<?php echo $k; ?>">
        ...
        <td align="left" width="60%">
            <a href="<?php echo $link; ?>"><?php echo htmlspecialchars($row->title, ENT_QUOTES, 'UTF-8'); ?></a><br />
                <?php
                if ($ext) {
                    if ($row->sectionid != 0)
        ...
        <td width="5%" class="td-state" align="center">
            <img id="img-pub-<?php echo $row->id;?>" obj_id="<?php echo $row->id;?>" obj_task="publish" obj_option="com_content" alt="<?php echo $title?>" src="<?php echo $img;?>"/>
        </td>
