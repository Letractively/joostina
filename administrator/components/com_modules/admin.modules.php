...
$database->setQuery($query);
$database->query();
// check needed to stop a module being assigned to `All` and other menu items resulting in a module being displayed twice
if(in_array('0',$menus)) {
...
// if a new record we must still prime the mosModule object with a default position and the order; also add an extra item to the order list to place the 'new' record in last position if desired
if($uid == 0) {
$row->position = 'left';
$row->showtitle = true;
//$row->ordering = $l;
...
/**
* Deletes one or more modules Also deletes associated entries in the #__module_menu table.
