...
function botUserContent(&$user) {
	...
	global  $mosConfig_live_site, $mosConfig_form_date_full, $mosConfig_absolute_path, $Itemid;
			...
			foreach ($user_items as $row) {
				...
				<tr class="sectiontableentry<?php echo ($k+1);?>">
					...
					<td align="center" <?php echo ($access->canPublish) ? 'onclick="ch_publ('.$row->id.');" class="td-state"' : null ;?>>
					    <img class="img-mini-state" src="<?php echo $img;?>" id="img-pub-<?php echo $row->id;?>" alt="Ïóáëèêàöèÿ" />
					</td>
	                <?php }?>
