...
return $this->debug_text($function_name.'database error: '.$this->db->getErrorMsg
().' ('.$this->db->getErrorNum().')');
}
...
return array('prevpage' => $rs->absolutepage() - 1, 'currentpage' => $rs->absolutepage
(), 'nextpage' => $rs->absolutepage() + 1, 'atfirstpage' => $rs->atfirstpage(),
'atlastpage' => $rs->atlastpage(), 'lastpageno' => $rs->lastpageno());
}
...
WHEREg2.group_id='.(int)$group_id;
break;
default:
$query .= '
WHEREg1.parent_id='.(int)$group_id;
}
$query .= '
ORDER BYg1.name';
...
$this->db->setQuery('
SELECTg.group_id,o.'.$group_type.'_id,gm.group_id AS member
FROM'.$object_table.' o
...
$this->db->setQuery('
SELECTg.group_id,o.'.$group_type.'_id,(gm.group_id IS NOT NULL) AS member
FROM'.$group_table.' g
...
($value).'\')
WHEREs.value=\''.$this->db->getEscaped($section_value).'\'');
$rows = $this->db->loadRowList();
if($this->db->getErrorNum()) {
...
$this->db->setQuery('
UPDATE'.$table.'
SETsection_value=\''.$this->db->getEscaped($section_value).'\',
...
SETvalue=\''.$this->db->getEscaped($value).'\',
section_value=\''.$this->db->getEscaped($section_value).'\'
WHEREsection_value=\''.$this->db->getEscaped($old[1]).'\'
ANDvalue=\''.$this->db->getEscaped($old[0]).'\'');
$this->db->query();
if(!$this->db->getErrorNum()) {
$this->debug_db('edit_object');
return false;
...
'.'
WHEREvalue IS NULL
ANDsection_value IS NULL
ANDgroup_id IS NULL
ANDa.id in ('.$sql_acl_ids.')');
