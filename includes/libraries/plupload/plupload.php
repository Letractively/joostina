...
// HTTP headers for no cache etc
	...
	private static function get_filefolder( $rootdir = false, $filename = false, $fileid = false ) {
		...
		return JPATH_BASE.DS.'attachments'.DS.$rootdir.DS. File::makefilename( $id );
	}

	public static function upload( $newnameforfile = false, $rootdir = false, $fileid = true, $show_result = true ) {
...
// Read binary input stream and append it to temp file
				...
				if ($in) {
					...
					if( is_file($targetfile) ) {
						...
						$file = new File( 0755 );
						$file_pach = self::get_filefolder( $rootdir, $targetfile, $fileid ).DS.$fileName;
...
// Return JSON-RPC response
		...
		return array( 'basename'=>$file_pach, 'livename'=>$file_live, 'location'=>$file_location );
	}
