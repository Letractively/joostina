			...
			if ( $params->get( 'intro_only' ) && $row->link_text ) {
				...
				<tr>
					<td align="left" colspan="2">
						<a href="<?php echo $row->link_on;?>" title="<?php echo $row->link_text;?>" class="readon<?php echo $params->get( 'pageclass_sfx' ); ?>">
		...
		if ( $params->get( 'item_navigation' ) && ( $task == 'view' ) && !$params->get( 'popup' ) && ( $row->prev || $row->next ) ) {
			...
			<tr>
				...
				if ( $row->prev ) {
					...
					<a href="<?php echo $row->prev; ?>" title="<?php echo $row->prev_title;?>">
							<?php echo _ITEM_PREVIOUS . $row->prev_title;
							 ?></a>
					</th>
					<?php
				}
				...
				if ( $row->next ) {
					...
					<a href="<?php echo $row->next; ?>" title="<?php echo $row->next_title;?>">
							<?php echo $row->next_title . _ITEM_NEXT;
							 ?></a>
					</th>
					<?php
				}
