...
class mosMainFrame {
		...
		*/
		...
		if($mosConfig_session_front) return;
	...
	*/
	...
	function login( $username=null,$passwd=null, $remember=0, $userid=NULL ) {
		global $acl, $_VERSION,$mosConfig_session_front,$mosConfig_auto_activ_login;
		if($mosConfig_session_front) return;
		$bypost = 0;
				...
				<?php
						...
						<td align="right" width="100%" class="buttonheading">
							<?php if(!$mosConfig_index_print) {?>
								<a href="<?php echo $link; ?>" target="_blank" onclick="window.open('<?php echo $link; ?>','win2','<?php echo $status; ?>'); return false;" title="<?php echo _CMN_PRINT;?>"><?php echo $image;?></a>
							<?php }else{?>
...
class joosXTabs {
		...
		*/
		function startTab( $tabText, $paneid ) {
				...
				echo '<a href="#" onclick=\'joostToggle("'.$paneid.'"); return false;\'><div class="joopage">'.$tabText.'</div></a>';
				echo "<div $hide id=\"".$paneid."\">";
		}
	...
	* @return string A select list
		*/
		function MenuSelect( $name='menuselect', $javascript=NULL ) {
				...
				$query = "SELECT params"
				. "\n FROM #__modules"
				. "\n WHERE module = 'mod_mainmenu' OR module = 'mod_mljoostinamenu'"
				;
				$database->setQuery( $query );
				$menus = $database->loadObjectList();
				$total = count( $menus );
	...
	<?php
		...
		*/
		function loadCalendar() {
				...
				<script type="text/javascript" src="<?php echo $mosConfig_live_site;?>/includes/js/calendar/lang/calendar-ru.js"></script>
				<?php
		}
		...
		function loadPquery($type=1){
			...
			if(!defined( '_PQ_LOADED' )){
				define( '_PQ_LOADED', 1 );
				echo '<script language="JavaScript" src="'.$mosConfig_live_site.'/includes/js/jquery/jquery.js" type="text/javascript"></script>';
		...
		function loadCodepress($location=null){
			...
			if(!defined( '_CODEPRESS_LOADED' )){
				...
				echo '<script language="JavaScript" src="'.$mosConfig_live_site.'/includes/js/codepress/codepress.js" type="text/javascript"></script>';
...
function mosChmod($path) {
	...
	$dirmode = NULL;
	if ($mosConfig_dirperms != '') $dirmode = octdec($mosConfig_dirperms);
	if (isset($filemode) || isset($dirmode)) return mosChmodRecursive($path, $filemode, $dirmode);
	return TRUE;
} // mosChmod
...
class joostina_api{
	...
	* $type - параметр конвертации, по умолчанию конвертируется из cp1251 в utf-8.
	**/
	function convert($text,$type = null){
		...
		return $NewEncoding->Convert($text, 'utf-8', 'windows-1251');
	}
	...
	*/
	function optimizetables() {
		...
		$flag = $mosConfig_absolute_path . '/cache/optimizetables.flag';
	...
	**/
	function RusTypo($text){
		global $mosConfig_absolute_path;
		require_once $mosConfig_absolute_path.'/administrator/components/com_rustypo/rustypo.php';
		return botRusTypo($text);
	}
}
