...
if( file_exists( $xmapComponentPath .'/language/' . $xmapLang . '.php') ) {
	...
	if ($task != 'ajax_request') {
		echo 'Языковой файл для [ '. $xmapLang .' ] не найден, используется: русский<br />';
	}
	$xmapLang = 'russian';
	require_once( $xmapComponentPath .'/language/russian.php' );
}
...
class XmapAdmin {
	...
	*/
	...
	function saveOptions( ) {
		...
		$file = $GLOBALS['mosConfig_absolute_path'] . '/components/com_xmap/css/xmap.css';
