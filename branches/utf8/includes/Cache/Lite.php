	...
	function _write($data){
		...
		if ($fp) {
			...
			if($mosConfig_cache_opt){
				$data = preg_replace('!/\*[^*]*\*+([^/][^*]*\*+)*/!', '', $data);// удаление комментариев
				$data = str_replace("\t", ' ', $data);// табуляцию иногда используют вместо пробелов - заменим на пробел
				$data = str_replace(array("\r\n", "\r", "\n", '  '), '', $data);// удаление табуляций, переводов строки и смещение каретки
				$data = str_replace('  ', ' ', $data);
				$data = str_replace('> <', '><', $data);
				$data = str_replace('{ ', '{', $data);// удаление лишних пробелов
				$data = str_replace(' }', '}', $data);
				$data = str_replace('; ', ';', $data);
				$data = str_replace(', ', ',', $data);
				$data = str_replace(' {', '{', $data);
