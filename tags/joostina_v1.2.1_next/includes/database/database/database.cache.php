...
class database {
	...
	*/
	function query() {
		...
		$this->dbcache->check_clear($this->_orig_sql, $this->_orig_prefix);
		return $this->_cursor;
	}
	...
	*/
	function getNumRows( $cur=null ) {
		$a = $this->dbcache->get_num_rows($this->_orig_sql);
		if($a != false) return $a;
	...
	*/
	function loadResult() {
		$a = $this->dbcache->get($this->_orig_sql, "loadResult", '');
		if($a != false) return $a;
		if (!($cur = $this->query())) {
			return null;
		}
		...
		$this->dbcache->set($this->_orig_sql, "loadResult",'', $ret, $this->_orig_prefix, $this->getNumRows());
		mysql_free_result( $cur );
		return $ret;
	}
	/**
	...
	*/
	function loadResultArray($numinarray = 0) {
		$a = $this->dbcache->get($this->_orig_sql, "loadResultArray", $numinarray);
		if($a != false) return $a;
		if (!($cur = $this->query())) {
			return null;
		}
		...
		$this->dbcache->set($this->_orig_sql, "loadResultArray",$numinarray, $array, $this->_orig_prefix, $this->getNumRows());
		mysql_free_result( $cur );
		return $array;
	}
	/**
	...
	*/
	function loadAssocList( $key='' ) {
		$a = $this->dbcache->get($this->_orig_sql, "loadAssocList", $key);
		if($a != false) return $a;
		if (!($cur = $this->query())) {
			return null;
		}
		...
		$this->dbcache->set($this->_orig_sql, "loadAssocList",$key, $array, $this->_orig_prefix, $this->getNumRows());
		mysql_free_result( $cur );
		return $array;
	}
	/**
	...
	*/
	function loadObjectList( $key='' ) {
		$a = $this->dbcache->get($this->_orig_sql, "loadObjectList", $key);
		if($a != false) return $a;
		if (!($cur = $this->query())) {
			return null;
		}
		...
		$this->dbcache->set($this->_orig_sql, "loadObjectList",$key, $array, $this->_orig_prefix, $this->getNumRows());
		mysql_free_result( $cur );
		return $array;
	}
	/**
	...
	*/
	function loadRow() {
		$a = $this->dbcache->get($this->_orig_sql, "loadRow",'');
		if($a != false) return $a;
		if (!($cur = $this->query())) {
			return null;
		}
		...
		$this->dbcache->set($this->_orig_sql, "loadRow",'', $ret, $this->_orig_prefix, $this->getNumRows());
		mysql_free_result( $cur );
		return $ret;
	}
	/**
	...
	*/
	function loadRowList( $key=null ) {
		$a = $this->dbcache->get($this->_orig_sql, "loadRowList", $key);
		if($a != false) return $a;
		if (!($cur = $this->query())) {
			return null;
		}
		...
		$this->dbcache->set($this->_orig_sql, "loadRowList",$key, $array, $this->_orig_prefix, $this->getNumRows());
		mysql_free_result( $cur );
		return $array;
	}
	/**
	...
	*/
	function stderr($showSQL = false) {
		return "Произошла ошибка $this->_errorNum"."<br /><font color=\"red\">$this->_errorMsg</font>".($showSQL?"<br />SQL = <pre>$this->_sql</pre>":'');
	}
...
class mosDBTable {
	...
	*/
	function checkout($user_id,$oid = null) {
		...
		if(intval($user_id)) {
			...
			$query = "UPDATE $this->_tbl"."\n SET checked_out = $user_id, checked_out_time = ".
				$this->_db->Quote($time)."\n WHERE $this->_tbl_key = ".$this->_db->Quote($this->
				$k);
			$this->_db->setQuery($query);
			...
			$query = "UPDATE $this->_tbl"."\n SET checked_out = 1, checked_out_time = ".$this->_db->Quote($time).
	...
	*/
	function checkin($oid = null) {
		...
		if($mosConfig_disable_checked_out) return true;
		if(!array_key_exists('checked_out',get_class_vars(strtolower(get_class($this))))) {
			$this->_error = "WARNING: ".strtolower(get_class($this)).
