...
<tr>
	<td>
		Команда:<select id="easysql_sel" class="text_area" onchange="changeQuery(this);">
		...
		<input class="button" type="button" value="Собрать SQL" onclick="changeQuery(this)" name="crsql"/>
	</td>
</tr>
<tr>
	<td>
	...
	if(!empty($rows)) {
		...
		foreach($rows as $row) {
			...
			if($_sel) $body .= '<td align=center nowrap>
					...
					'\';" href="#">	<img border=0 src="../images/M_images/edit.png" alt="Редактировать" /></a>&nbsp;
					<a onclick="if (confirm(\'Удалить эту запись ?\')) {this.href=\'index2.php?option=com_easysql&task=delete&prm1='.
					base64_encode($table).'&key='.$key.'&id='.$row[$key].'&prm2='.base64_encode($query).
					'\'};" href="#"><img border=0 src="images/publish_x.png" alt="Удалить" /></a></td>';
...
<form id="adminForm" action="index2.php?option=com_easysql" method="post" name="adminForm">
	<table class="adminheading">
		<tr>
			<th>
				<?php echo "$table [ $key = $id ]"; ?>:<small><?php echo $task == 'edit' ? 'Редактирование' : 'Создание'; ?></small>
			</th>
		</tr>
		</table>
