...
class mosToolBar {
	...
	*/
	function publishList( $task='publish', $alt=_CMN_PUBLISHED ) {
		...
		?>
		<td>
			<a class="toolbar" href="javascript:if (document.adminForm.boxchecked.value == 0){ alert('Выберите объект из списка для его публикации'); } else {submitbutton('<?php echo $task;?>', '');}" >
				<?php echo $image; ?></a>
		</td>
		<?php
	}
	...
	*/
	function unpublishList( $task='unpublish', $alt=_CMN_UNPUBLISHED ) {
		...
		?>
		<td>
			<a class="toolbar" href="javascript:if (document.adminForm.boxchecked.value == 0){ alert('Выберите объект из списка для отмены его публикации'); } else {submitbutton('<?php echo $task;?>', '');}" >
				<?php echo $image; ?></a>
		</td>
		<?php
	}
	...
	*/
	function archiveList( $task='archive', $alt=_CMN_ARCHIVE ) {
		...
		?>
		<td>
			<a class="toolbar" href="javascript:if (document.adminForm.boxchecked.value == 0){ alert('Выберите объект из списка для перемещения в архив'); } else {submitbutton('<?php echo $task;?>', '');}" >
				<?php echo $image; ?></a>
		</td>
		<?php
	}
	...
	*/
	function unarchiveList( $task='unarchive', $alt=_CMN_UNARCHIVE ) {
		...
		?>
		<td>
			<a class="toolbar" href="javascript:if (document.adminForm.boxchecked.value == 0){ alert('Выберите материал для восстановления его из архива'); } else {submitbutton('<?php echo $task;?>', '');}" >
				<?php echo $image; ?></a>
		</td>
		<?php
	}
	...
	*/
	function editList( $task='edit', $alt=_E_EDIT ) {
		...
		?>
		<td>
			<a class="toolbar" href="javascript:if (document.adminForm.boxchecked.value == 0){ alert('Выберите объект из списка для его редактирования'); } else {submitbutton('<?php echo $task;?>', '');}" >
				<?php echo $image; ?></a>
		</td>
		<?php
	}
	...
	*/
	function deleteList( $msg='', $task='remove', $alt=_CMN_DELETE ) {
		...
		?>
		<td>
			<a class="toolbar" href="javascript:if (document.adminForm.boxchecked.value == 0){ alert('Выберите объект из списка для его удаления'); } else if (confirm('Вы действительно хотите удалить выбранные объекты? <?php echo $msg;?>')){ submitbutton('<?php echo $task;?>');}">
				<?php echo $image; ?></a>
		</td>
		<?php
	}
